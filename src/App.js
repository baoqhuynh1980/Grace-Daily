...
const getSermonContent = (topic) => {
  ...
  return { mainMessage: `${topic} is a powerful subject in the Christian faith. God's Word has much to say about this topic.`, keyTakeaways: [`Seek God's wisdom on ${topic} through prayer and His Word.`, `Apply what you learn about ${topic} to your everyday life.`], scriptures: ["Proverbs 3:5-6 — Trust in the Lord with all your heart.", "Psalm 119:105 — Your word is a lamp to my feet.", "James 1:5 — If any of you lacks wisdom let him ask God."], discussionQuestions: [`What does God's Word say about ${topic}?`, `How does ${topic} apply to your current season?`], prayer: `Father in the name of Jesus Christ give me wisdom about ${topic}. In the name of Jesus Christ. Amen.`, journal: `Write about what God is showing you about ${topic}. What will you apply this week?` };
};
  ...
  return (
    <div style={s.app}>
      ...
      <div style={s.content}>
        ...
        {activeTab === "bible" && (
          <div>
            {selectedBook && selectedChapter && (<div><button style={s.backBtn} onClick={() => { setSelectedChapter(null); setChapterText(null); }}>← Back to {selectedBook.name}</button><div style={s.cardGold}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div><p style={{ color: GOLD_LIGHT, fontSize: 11, fontFamily: "sans-serif", margin: "0 0 4px", letterSpacing: 1, textTransform: "uppercase" }}>{selectedBook.name}</p><h2 style={{ color: WHITE, fontSize: 20, margin: 0 }}>Chapter {selectedChapter}</h2></div><div style={{ display: "flex", gap: 6 }}>{bibleVersions.map(v => (<button key={v.id} onClick={() => { setBibleVersion(v.id); loadChapter(selectedBook, selectedChapter); }} style={{ background: bibleVersion === v.id ? GOLD : "rgba(255,255,255,0.2)", border: "none", borderRadius: 8, padding: "4px 10px", color: WHITE, fontSize: 11, cursor: "pointer", fontFamily: "sans-serif", fontWeight: bibleVersion === v.id ? "bold" : "normal" }}>{v.name}</button>))}</div></div></div>{bibleLoading && <div style={{ ...s.card, textAlign: "center", padding: 32 }}><div style={{ fontSize: 28, marginBottom: 8 }}>📖</div><p style={{ color: BROWN, fontStyle: "italic" }}>Loading chapter...</p></div>}{chapterText && chapterText.error && <div style={s.card}><p style={{ color: BROWN, textAlign: "center" }}>{chapterText.error}</p></div>}{chapterText && chapterText.verses && (<div><div style={s.card}>{chapterText.verses.map((v, i) => (<div key={i} style={{ display: "flex", gap: 10, marginBottom: 12, alignItems: "flex-start" }}><span style={{ color: GOLD, fontSize: 11, fontWeight: "bold", fontFamily: "sans-serif", minWidth: 20, marginTop: 2 }}>{v.verse}</span><p style={{ color: BROWN_DARK, fontSize: 15, lineHeight: 1.8, margin: 0, flex: 1 }}>{v.text}</p></div>))}</div>{isChapterRead(selectedBook, selectedChapter) ? (<div style={{ ...s.card, background: GOLD_LIGHT, textAlign: "center" }}><p style={{ color: GOLD, fontSize: 15, fontWeight: "bold", margin: "0 0 4px", fontFamily: "sans-serif" }}>✅ Chapter Completed!</p><p style={{ color: BROWN, fontSize: 13, margin: 0 }}>This chapter is marked as read. Keep going! 🔥</p></div>) : (<button style={s.btn} onClick={markChapterRead}>{user ? "✅ Mark as Read" : "Sign in to Track Progress"}</button>)}<div style={{ display: "flex", gap: 8, marginTop: 8 }}>{selectedChapter > 1 && <button style={{ ...s.btnOutline, flex: 1 }} onClick={() => loadChapter(selectedBook, selectedChapter - 1)}>← Previous</button>}{selectedChapter < selectedBook.chapters && <button style={{ ...s.btn, flex: 1, marginTop: 0 }} onClick={() => loadChapter(selectedBook, selectedChapter + 1)}>Next Chapter →</button>}</div></div>)}</div>)}
